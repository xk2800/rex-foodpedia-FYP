<!DOCTYPE html>
<html>
<?php
    include("../db-connect.php");
?>

    <head><title>You hit a 500| REX Foodipedia</title>
	
	<link rel="icon" type="image/png" href= "image/MYRUN 1.png">

    <meta name="viewpoint" content="width=device-width, initial-scale=1">
    <meta name="viewpoint" content="width=device-width">

<!--FONTS.CSS-->
    <link rel="preload" href="css/fonts.css" as="style">

<!--THIS IS FONT AWESOME JAVASCRIPT START-->
    <script src="https://kit.fontawesome.com/daa253e478.js" crossorigin="anonymous"></script>
<!--THIS IS FONT AWESOME JAVASCRIPT END-->

<!--THIS IS BOOTSTRAP CSS PART START-->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
<!--THIS IS BOOTSTRAP CSS PART END-->
    
<link rel="stylesheet" href="../css/404.css">

    <style>
        body{
            background-color: #e8ded2;
            font-family: 'Bangers', cursive;
        }
    </style>

</head>
<body>
<div id="overlay">
<!--START CODE BETWEEN THIS 2 DIV, if not, navbar will error-->

    <div class="container">
        <a href="index"><img src="../img/logo/logo.png"></a>

        <div class="opps">
            <p>Opps, looks like there was an error from us.</p>
            <p>Don't worry, we are on attempting to fix it. Check on us again later.</p>
        </div>





    </div>



</div>
<!--THIS IS BOOTSTRAP JAVASRIPT PART START-->
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>   
    <script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
<!--THIS IS BOOTSTRAP JAVASCRIPT PART END-->

</body>
</html>